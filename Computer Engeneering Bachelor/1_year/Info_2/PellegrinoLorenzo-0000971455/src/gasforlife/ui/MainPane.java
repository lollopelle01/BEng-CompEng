...
public class MainPane extends BorderPane {
  ...
  private void computeShare()
  {
	 ...
	 if(this.radioAnnual.isSelected()) {
		 bill=new Bill(BillingFrequency.ANNUAL, cost, fCost, vCost, cons, cm3, em3, Optional.empty());
	 }
	 else if(this.radioMonthly.isSelected()) {
		 bill=new Bill(BillingFrequency.MONTHLY, cost, fCost, vCost, cons, cm3, em3, Optional.of(Month.from(this.pickerMonth.getValue())));
	 }
	 else {
		 GasForLifeApp.alert("problema!", "nessun valore selezionato tra mensile e annuale", "riprova");
	 }
	 ...
	 StringBuilder sb=new StringBuilder();
	 {
		 ...
		 sb.append("La differenza tra il consumo presunto nella bolletta e quello reale risulta essere di m3: "+this.controller.getDiffCons(bill));
	 }
