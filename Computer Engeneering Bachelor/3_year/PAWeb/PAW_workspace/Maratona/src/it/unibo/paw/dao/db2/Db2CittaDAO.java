...
public class Db2CittaDAO implements CittaDAO { // NON AGGIUNGERE UNIMPLEMENTED METHODS, SCRIVI PRIMA QUELLI CHE CI SONO
	...
	private static final String find_maratona_by_citta = 
				"SELECT m.* " + 
				"FROM Maratona m, Citta c " + 
				"WHERE m.idCitta = c.id ";
	
	private static final String query_specifica = 
			"SELECT count(m.id) " + 
			"FROM Maratona m, Citta c " + 
			"WHERE m.idCitta = c.id AND m.tipo = ? AND c.nazione =  ?";
	...
	public void create(CittaDTO c) {
		...
		try {
			...
			prep_stmt.setInt(1, c.getId());
			prep_stmt.setString(2, c.getNome());
			prep_stmt.setString(3, c.getNazione());
			// --- c. Esegui l'azione sul database ed estrai il risultato (se atteso)
	...
	public CittaDTO read(int id) {
		...
		try {
			...
			// --- d. Cicla sul risultato (se presente) pe accedere ai valori di ogni sua tupla
			if (rs.next()) {
				CittaDTO entry = new CittaDTO();
				entry.setId(rs.getInt(ID));
				entry.setNome(rs.getString(NOME));
				entry.setNazione(rs.getString(NAZIONE));
				entry.setMaratone(getMaratoneByCitta(id));
	...
	public boolean update(CittaDTO c) {
		...
		try {
			...
			// --- b. Pulisci e imposta i parametri (se ve ne sono)
			prep_stmt.clearParameters();
			prep_stmt.setString(1, c.getNome());
			prep_stmt.setString(2, c.getNazione());
			prep_stmt.setInt(3, c.getId());
			// --- c. Esegui l'azione sul database ed estrai il risultato (se atteso)
			prep_stmt.executeUpdate();
			// --- d. Cicla sul risultato (se presente) pe accedere ai valori di ogni sua tupla
	...
	public Set<MaratonaDTO> getMaratoneByCitta(int idCitta) {
		// --- 1. Dichiarazione della variabile per il risultato ---
		Set<MaratonaDTO> result = new HashSet<MaratonaDTO>();
		// --- 2. Controlli preliminari sui dati in ingresso ---
		if (idCitta < 0) {
		...
		try {
			// --- a. Crea (se senza parametri) o prepara (se con parametri) lo statement
			PreparedStatement prep_stmt = conn.prepareStatement(find_maratona_by_citta);
			// --- b. Pulisci e imposta i parametri (se ve ne sono)
			prep_stmt.clearParameters();
			prep_stmt.setInt(1, idCitta);
			// --- c. Esegui l'azione sul database ed estrai il risultato (se atteso)
			ResultSet rs = prep_stmt.executeQuery();
			// --- d. Cicla sul risultato (se presente) pe accedere ai valori di ogni sua tupla
			while (rs.next()) {
		...
		// --- 5. Gestione di eventuali eccezioni ---
			...
			System.err.println("find(): failed to retrieve entry with idCitta = " + idCitta + ": " + e.getMessage());
			e.printStackTrace();
		}
		// --- 6. Rilascio, SEMPRE E COMUNQUE, la connessione prima di restituire il controllo al chiamante
		finally {
	...
	public int getNumMaratoneSpecifiche(String tipo, String nazione) {
		...
		if (nazione == null | nazione.equals("")) {
			System.err.println("getNumMaratoneSpecifiche(): nazione is invalid");
			return result;
		}
		
		if (tipo == null | tipo.equals("")) {
			System.err.println("getNumMaratoneSpecifiche(): tipo is invalid");
			return result;
		}
		// --- 3. Apertura della connessione ---
		Connection conn = Db2DAOFactory.createConnection();
		...
		try {
			// --- a. Crea (se senza parametri) o prepara (se con parametri) lo statement
			PreparedStatement prep_stmt = conn.prepareStatement(query_specifica);
			// --- b. Pulisci e imposta i parametri (se ve ne sono)
			prep_stmt.clearParameters();
			prep_stmt.setString(1, tipo);
			prep_stmt.setString(2, nazione);
			// --- c. Esegui l'azione sul database ed estrai il risultato (se atteso)
			ResultSet rs = prep_stmt.executeQuery();
			// --- d. Cicla sul risultato (se presente) pe accedere ai valori di ogni sua tupla
			rs.next();
			result = rs.getInt(1); System.out.println(result);
			// --- e. Rilascia la struttura dati del risultato
			rs.close();
			// --- f. Rilascia la struttura dati dello statement
			prep_stmt.close();
		...
		// --- 5. Gestione di eventuali eccezioni ---
			...
			System.err.println("find(): failed to retrieve entry with nazione = " + nazione + ": " + e.getMessage());
			e.printStackTrace();
		}
		// --- 6. Rilascio, SEMPRE E COMUNQUE, la connessione prima di restituire il controllo al chiamante
		finally {
