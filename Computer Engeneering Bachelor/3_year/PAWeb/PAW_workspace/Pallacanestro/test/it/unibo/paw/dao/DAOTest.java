...
public class DAOTest {
	...
	public static void main(String[] args) {
		...
		tDAO.create(idS2, idG1); tDAO.create(idS2, idG2); tDAO.create(idS2, idG3);
		tDAO.create(idS3, idG1); tDAO.create(idS3, idG2); 
		tDAO.create(idS4, idG1);
		
		// Scriviamo risultato
		try {
			FileWriter fw = new FileWriter("Pallacanestro.txt");
			fw.write("Ad ogni giocatore i suoi allenatori:\n\n");
			// Per ogni giocatore
			for(GiocatoreDTO giocatore : tDAO.getGiocatoriBySquadra(idS1)) {
				fw.write("Per quanto riguarda " + giocatore.getNome() + ":\n");
				// Stampare nome allenatore
				for(Squadra_PallacanestroDTO squadra : tDAO.getSquadreByGiocatore(giocatore.getId())) {
					fw.write("\t- " + squadra.getAllenatore() + "\n");
				}
			}
			fw.close();
		} catch (IOException e) {
