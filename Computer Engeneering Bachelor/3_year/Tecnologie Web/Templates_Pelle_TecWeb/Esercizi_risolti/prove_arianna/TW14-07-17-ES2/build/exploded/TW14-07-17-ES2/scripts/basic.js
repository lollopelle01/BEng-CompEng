...
function toUpperCaseAJAX(theElement, theXhr) {
	...
	var 
		fileName = MyGetElementById("fileName").value;
		text = MyGetElementById("text").value;
	
	var argument = "fileName="+fileName+"&text="+text;
...
function checkText() {
	...
	if(ok) {
		var corrButton = MyGetElementById("secret");
		corrButton.style.display = 'block';
	}
}
