...
NAME, VERSION, GITHUB, AUTHOR, LICENSE = "Sec Lab Esame 29 Ottobre (2021) < Path Traversal Challenge", "0.01", "https://gitlab.com/wild_boar/labsec_course", "@wild_boar", "GPLv3"
        ...
        try:
            if path == '/':
                if "path" in params:
                    ...
                    if re.search("^\.\./",paths):
                        content = "Errore! Carattere filtrato"
                    elif re.search("^/\.\.",paths):
                        content = "Errore! Errore! Carattere filtrato"
                    elif re.search("^/[a-zA-Z0-9]+",paths):
                        content = "Errore! Abbiamo disabilitato la possibilita' di invocare direttamente il file /etc/passwd"                        
                ...
                if HTML_PREFIX in content and HTML_POSTFIX not in content:
                    content +="<div><span>Web Challenge Esame 29 Ottobre 2021:</span></div>\n"
