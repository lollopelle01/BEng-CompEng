...
int main(int argc, char **argv) {
    ...
    char               identificatore[WORD_LENGTH], persone[WORD_LENGTH];
    udp_req_t          req;
    ...
    while (scanf("%s", identificatore) == 1) {
        strcpy(req.id, identificatore);
        printf("Nuovo numero di persone: ");
        scanf("%s", persone);
        strcpy(req.new_people, persone);

        printf("Invio richiesta di cambio numero delle persone in %s, alla prenotazione %s.\n", req.new_people,req.id);
        ...
        if (ris < 0) {
            printf("Errore: non esiste nessuna prenotazione con questo id o errore in apertura\n");
        } else {
            // se ris = 0 giusto  
