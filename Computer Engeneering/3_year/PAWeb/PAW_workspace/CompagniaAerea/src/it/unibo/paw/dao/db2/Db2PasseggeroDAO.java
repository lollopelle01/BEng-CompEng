...
public class Db2PasseggeroDAO implements PasseggeroDAO {
	...
	static final String TABLE = "Passeggero";

	// -------------------------------------------------------------------------------------

	static final String ID = "id";
	static final String CODPASSEGGERO = "codPasseggero";
	static final String NOME = "nome";
	static final String COGNOME = "cognome";
	static final String NUMPASSAPORTO = "numPassaporto";
	...
	static String create = "CREATE " +
			...
			CODPASSEGGERO + " VARCHAR(100) NOT NULL, " +
			NOME + " VARCHAR(100) NOT NULL, " +
			COGNOME + " VARCHAR(100) NOT NULL, " +
			NUMPASSAPORTO + " INT NOT NULL " +
			") ";
	...
	public void create(PasseggeroDTO p) {
		...
		try {
			...
			prep_stmt.setString(2, p.getCodPasseggero());
			prep_stmt.setString(3, p.getNome());
			prep_stmt.setString(4, p.getCognome());
			prep_stmt.setInt(5, p.getNumPassaporto());
	...
	public PasseggeroDTO read(int id) {
		...
		try {
			...
			if (rs.next()) {
				// Creo risultato
				PasseggeroDTO entry = new PasseggeroDTO();
				entry.setId(rs.getInt(ID));
				entry.setCodPasseggero(rs.getString(CODPASSEGGERO));
				entry.setNome(rs.getString(NOME));
				entry.setCognome(rs.getString(COGNOME));
				entry.setNumPassaporto(rs.getInt(NUMPASSAPORTO));
				
				// Eventualmente uso classi di mapping
				Db2TemplateMappingDAO csmdao = new Db2TemplateMappingDAO();
				entry.setVoli(csmdao.voliByPasseggero(entry.getId()));
				result = entry;
			}
			rs.close();
			prep_stmt.close();
	...
	public boolean update(PasseggeroDTO p) {
		...
		try {
			...
			prep_stmt.clearParameters();
			prep_stmt.setString(1, p.getCodPasseggero());
			prep_stmt.setString(2, p.getNome());
			prep_stmt.setString(3, p.getCognome());
			prep_stmt.setInt(4, p.getNumPassaporto());
			prep_stmt.setInt(5, p.getId());
			prep_stmt.executeUpdate();
			result = true;
			prep_stmt.close();
		} catch (Exception e) {
