...
public class ServiceServlet2 extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		System.out.println("Servlet2: " + matrix.toString());
		
		//delegazione a un thread//
		ExecThread thread = new ExecThread(3, matrix);
		...
		try {
			...
			System.out.println("Servlet2: result: " +thread.isResult());
			
			if(thread.isResult()) {
				result.setSomma(thread.getSomma());
				System.out.println("Servlet2: SOMMMA: " +thread.getSomma());
			}
			
			resp.getWriter().write(gson.toJson(result));
			
		} catch (InterruptedException e) {
			System.out.println("Servlet2: Thread interrupted");
			e.printStackTrace();
