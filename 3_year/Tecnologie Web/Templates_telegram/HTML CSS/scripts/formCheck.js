function formCheck(form) {
	...
	// if (form.elements.login.value.length != 12) {
	// 	return false;
	// }
...
function spawnImages(n, category) {
	myGetElementById("registrationForm").setAttribute("hidden", "hidden");

	var div = myGetElementById('visualizza');
	div.innerHTML="";
	for (var i =0; i<n; i++) {
		var input = document.createElement('img');
		input.setAttribute('src', category+i+".jpg");
		input.setAttribute('alt', category+i+".jpg not found");
...
function showRegistrationForm() {
	myGetElementById("visualizza").innerHTML = "";
	myGetElementById("registrationForm").removeAttribute("hidden");
}

function checkNotNumber(element) {
	if (/[0-9]/.test(element.value)) {
		alert("Il campo "+element.getAttribute("name")+" non e' valido")
		//element.focus();
		return false;
	}
	return true;
...
function checkPassword(field) {
    ...
    if(numeri<3||maiuscola<1||speciale<1||lettere<4){
        alert("Pwd deve contenere 3 numeri, 1 lettera maiuscola, 4 lettere totali e un carattere speciale tra !, $ e ?")
				return false;
		} else
			return true;
}
