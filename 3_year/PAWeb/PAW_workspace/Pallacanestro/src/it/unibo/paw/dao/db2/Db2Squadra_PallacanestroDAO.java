...
public class Db2Squadra_PallacanestroDAO implements Squadra_PallacanestroDAO { // NON AGGIUNGERE UNIMPLEMENTED METHODS, SCRIVI PRIMA QUELLI CHE CI SONO
	// === Costanti letterali per non sbagliarsi a scrivere !!! ============================

	private static final String TABLE = "Squadra";
	...
	public int create(Squadra_PallacanestroDTO squadra) {
		...
		try {
			...
			prep_stmt.setInt(1, result); //ID BROKER
	 ...
	 * R
	 */
	public Squadra_PallacanestroDTO read(int id) {
		// --- 1. Dichiarazione della variabile per il risultato ---
		Squadra_PallacanestroDTO result = null;
		// --- 2. Controlli preliminari sui dati in ingresso ---
		if (id < 0) {
		...
		try {
			...
			// --- d. Cicla sul risultato (se presente) pe accedere ai valori di ogni sua tupla
			if (rs.next()) {
				Squadra_PallacanestroDTO entry = new Squadra_PallacanestroDTO();
				...
				entry.setGiocatori(csmdao.getGiocatoriBySquadra(entry.getId()));
				result = entry;
			}
			// --- e. Rilascia la struttura dati del risultato
			rs.close();
	...
	public boolean update(Squadra_PallacanestroDTO squadra) {
		...
		try {
			...
			prep_stmt.setInt(4, squadra.getId());
			// --- c. Esegui l'azione sul database ed estrai il risultato (se atteso)
			prep_stmt.executeUpdate();
			// --- d. Cicla sul risultato (se presente) pe accedere ai valori di ogni sua tupla
